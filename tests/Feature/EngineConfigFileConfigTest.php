...
/**
 * `engine.config` points the analyser at the application's own PHPStan config. Configuring one that
 ...
 */
it('warns, portably, when engine.config names a file that is not there', function (): void {
    setBuild('engine.config', 'phpstan.neon');

    $result = app(DocumentBuilder::class)->build('default', WorkbenchEngine::make());

    $warnings = diagnosticsCoded($result->diagnostics, 'config.engine-config-missing');
    expect($warnings)->toHaveCount(1)
        ...
        ->and($warnings[0]->help)->toContain('docuccino.yaml')
...
it('says nothing about a file that is there', function (): void {
    ...
    setBuild('engine.config', $neon);

    $result = app(DocumentBuilder::class)->build('default', WorkbenchEngine::make());

    expect(diagnosticsCoded($result->diagnostics, 'config.engine-config-missing'))->toBe([]);
