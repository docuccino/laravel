 ...
 * sits inside it exactly as `resources/docs/api` sits inside a real application. That also exercises the
 * project-root-relative page `source` prefix the compiler promises, which nothing else covers.
 */
...
it('strips the whole content layer from OpenAPI (byte-identical to the content-free golden)', function (): void {
    ...
    // …and the OpenAPI emission drops it, matching the content-free golden byte for byte.
...
it('keeps content out of the fragment cache key (a prose edit does not invalidate fragments; an env change does)', function (): void {
    ...
    // Editing a content file produces no new fragment cache keys: operation fragments never read content,
    // so a prose edit is a warm hit across every route — content is picked up by the always-fresh
    ...
    // A real document-level environment change (app.url feeds Passport oauth2 flow URLs) does move the env
    // digest, so fresh fragment cache keys appear.
